...
import { 
  ...
  Key, Globe2, Upload, Languages, ShieldCheck, TriangleAlert
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const updateNodeData = useCallback((nodeId: string, key: string, value: any) => {
    ...
    setValidationErrors([]);
    setWorkflow(prev => prev ? ({ ...prev, nodes: prev.nodes.map(n => n.id === nodeId ? { ...n, data: { ...n.data, [key]: value } } : n), isDirty: true }) : null);
  }, [selectedRunId]);
  ...
  /* Fix typo 'portPort' to 'portId' at line 424 */
  ...
  const addNode = useCallback((tool: ToolDefinition, x?: number, y?: number, dataOverride?: Record<string, any>) => {
    ...
    if (tool.id === 'text-to-image' && !defaultData.aspectRatio) defaultData.aspectRatio = "1:1";
  ...
  const disconnectedInputs = useMemo(() => {
    if (!workflow) return [];
    const list: { nodeId: string; port: Port; toolName: string }[] = [];
    workflow.nodes.forEach(node => {
      const tool = TOOLS.find(t => t.id === node.toolId);
      if (!tool || tool.category === 'Input') return;
      tool.inputs.forEach(port => { 
        // strictly check if NO connection target is this port
        const isConnected = workflow.connections.some(c => c.targetNodeId === node.id && c.targetPortId === port.id);
        if (!isConnected) {
           list.push({ nodeId: node.id, port, toolName: (lang === 'zh' ? tool.name_zh : tool.name) }); 
        }
      });
    });
    return list;
  ...
  const runWorkflow = async (startNodeId?: string, onlyOne?: boolean) => {
    ...
    if (startNodeId) {
      ...
      if (workflow.history.length > 0) {
        Object.entries(workflow.history[0].outputs).forEach(([nodeId, val]) => {
          if (!nodesToRunIds.has(nodeId) && !sessionOutputs[nodeId]) sessionOutputs[nodeId] = val;
        });
      }
    }
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-slate-950/90 backdrop-blur-2xl animate-in fade-in duration-300">
          <div className="bg-slate-900 border border-slate-800 rounded-[40px] shadow-2xl w-full max-w-5xl h-full flex flex-col relative overflow-hidden">
            <div className="p-6 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-4"><h2 className="text-xl font-black uppercase tracking-widest">{expandedResultData.label}</h2></div>
              <div className="flex items-center gap-3"><button onClick={() => downloadFile(expandedResultData.content, expandedResultData.label, expandedResultData.type)} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-2xl transition-all active:scale-90"><Download size={20}/></button><button onClick={() => setExpandedOutput(null)} className="p-3 text-slate-400 hover:text-white transition-all"><X size={24}/></button></div>
            </div>
            ...
            {sourceNodes.map(node => {
              ...
              return (
                <div key={node.id} className={`node-element absolute w-56 bg-slate-900 border rounded-3xl shadow-2xl transition-all z-10 group ${isSelected ? 'border-indigo-500 ring-8 ring-indigo-500/10' : 'border-slate-800'}`} style={{ left: node.x, top: node.y }} onClick={e => { 
                  ...
                  setSelectedConnectionId(null); 
                }} onMouseDown={e => { const world = screenToWorld(e.clientX - (canvasRef.current?.getBoundingClientRect().left || 0), e.clientY - (canvasRef.current?.getBoundingClientRect().top || 0)); setDraggingNode({ id: node.id, offsetX: world.x - node.x, offsetY: world.y - node.y }); }}>
           ...
           {selectedNodeId && selectedNode ? (
             <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                ...
                <div className="space-y-6">
                  ...
                  {selectedNode.toolId === 'text-prompt' && <textarea value={selectedNode.data.value || ''} onChange={e => updateNodeData(selectedNode.id, 'value', e.target.value)} className="w-full h-40 bg-slate-800/40 border border-slate-700/50 rounded-2xl p-4 text-xs resize-none focus:border-indigo-500 transition-all" placeholder="Input context..."/>}
                  
                  {selectedNode.toolId === 'image-input' && (
                    <div className="space-y-4">
                      <div className="flex flex-wrap gap-2">
                        {(selectedNode.data.value || []).map((img: string, i: number) => (
                          <div key={i} className="relative w-16 h-16 group">
                            <img src={img} className="w-full h-full object-cover rounded-lg border border-slate-700" />
                            <button onClick={() => {
                              const next = selectedNode.data.value.filter((_: any, idx: number) => idx !== i);
                              updateNodeData(selectedNode.id, 'value', next);
                            }} className="absolute -top-1 -right-1 p-0.5 bg-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X size={10}/></button>
                          </div>
                        ))}
                        <label className="w-16 h-16 flex items-center justify-center border border-dashed border-slate-700 rounded-lg cursor-pointer hover:border-indigo-500 transition-colors">
                          ...
                          <input type="file" multiple accept="image/*" className="hidden" onChange={async (e) => {
                            ...
                            updateNodeData(selectedNode.id, 'value', [...(selectedNode.data.value || []), ...base64s]);
                          }} />
                        </label>
                      </div>
                    </div>
                  ...
                  {selectedNode.toolId === 'audio-input' && (
                    <div className="space-y-4">
                      {selectedNode.data.value ? (
                        <div className="p-4 bg-slate-800 rounded-2xl border border-slate-700 flex flex-col gap-2">
                           <audio controls src={selectedNode.data.value} className="w-full h-8" />
                           <button onClick={() => updateNodeData(selectedNode.id, 'value', null)} className="text-[10px] text-red-400 font-bold uppercase hover:text-red-300">{t('remove_audio')}</button>
                        </div>
                      ) : (
                        <label className="w-full h-32 flex flex-col items-center justify-center border border-dashed border-slate-700 rounded-2xl cursor-pointer hover:border-indigo-500 transition-colors">
                          <Volume2 size={32} className="text-slate-600 mb-2" />
                          <span className="text-xs text-slate-500">{t('upload_audio')}</span>
                          <input type="file" accept="audio/*" className="hidden" onChange={async (e) => {
                            ...
                            if (file) {
                              ...
                              updateNodeData(selectedNode.id, 'value', base64);
                            }
                          }} />
                        </label>
                      )}
                  ...
                  {selectedNode.toolId === 'video-input' && (
                    <div className="space-y-4">
                      {selectedNode.data.value ? (
                        <div className="relative group">
                           <video controls src={selectedNode.data.value} className="w-full rounded-2xl border border-slate-700" />
                           <button onClick={() => updateNodeData(selectedNode.id, 'value', null)} className="absolute top-2 right-2 p-1.5 bg-slate-900/80 rounded-full text-red-400 hover:bg-slate-900 transition-colors"><X size={14}/></button>
                        </div>
                      ) : (
                        <label className="w-full h-32 flex flex-col items-center justify-center border border-dashed border-slate-700 rounded-2xl cursor-pointer hover:border-indigo-500 transition-colors">
                          <VideoIcon size={32} className="text-slate-600 mb-2" />
                          <span className="text-xs text-slate-500">{t('upload_video')}</span>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <select value={selectedNode.data.mode} onChange={e => updateNodeData(selectedNode.id, 'mode', e.target.value)} className="w-full bg-slate-800 rounded-xl p-3 text-xs border border-slate-700">
                          {['basic', 'enhance', 'summarize', 'polish'].map(m => <option key={m} value={m}>{m.toUpperCase()}</option>)}
                        </select>
                      </div>
                      <div className="space-y-2">
                        ...
                        {selectedNode.data.customOutputs.map((o:any, i:number) => (
                          <div key={i} className="p-3 bg-slate-950/40 border border-slate-800 rounded-xl flex items-center gap-2">
                            <input value={o.id} onChange={e => { const n = [...selectedNode.data.customOutputs]; n[i].id = e.target.value; updateNodeData(selectedNode.id, 'customOutputs', n); }} className="bg-transparent border-none text-[10px] font-black text-indigo-400 flex-1 p-0"/>
                            <button onClick={() => { const n = selectedNode.data.customOutputs.filter((_:any,idx:number)=>idx!==i); updateNodeData(selectedNode.id, 'customOutputs', n); }}><X size={12}/></button>
                          </div>
                        ))}
                        <button onClick={() => { const n = [...selectedNode.data.customOutputs, { id: `out_${Date.now().toString().slice(-4)}`, label: 'Output' }]; updateNodeData(selectedNode.id, 'customOutputs', n); }} className="w-full py-2 border border-dashed border-slate-700 text-[10px] text-slate-500 uppercase hover:text-indigo-400 transition-all">+ {t('add_output')}</button>
                      </div>
                    </div>
                  )}
                     ...
                     <div className="space-y-4">
                        {disconnectedInputs.map(item => (
                           <div key={`${item.nodeId}-${item.port.id}`} className="space-y-2 p-5 bg-slate-800/20 border border-slate-800 rounded-[32px]">
                              ...
                              <textarea value={workflow.globalInputs[`${item.nodeId}-${item.port.id}`] || ''} onChange={e => handleGlobalInputChange(item.nodeId, item.port.id, e.target.value)} className="w-full h-24 bg-slate-900 border border-slate-800 rounded-xl p-3 text-xs resize-none focus:border-indigo-500 transition-all custom-scrollbar" />
                           </div>
                        ))}
                     </div>
                   )}
      ...
      <footer className="h-80 border-t border-slate-800/60 bg-slate-900/60 backdrop-blur-3xl z-40 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-x-auto p-8 flex gap-8 items-start custom-scrollbar">
           {activeResultsList.length === 0 && !workflow.isRunning ? (
             ...
             return (
               <div key={node.id} className={`min-w-[320px] max-w-[400px] bg-slate-900/50 rounded-[32px] border p-6 flex flex-col shadow-2xl relative overflow-hidden group transition-all h-[180px] ${node.status === NodeStatus.ERROR ? 'border-red-500/30 bg-red-500/5' : isTerminal ? 'border-emerald-500/30' : 'border-slate-800/60'}`}>
                  ...
                  <div className={`flex-1 overflow-hidden rounded-xl p-3 ${node.status === NodeStatus.ERROR ? 'bg-red-500/10 border border-red-500/20' : 'bg-slate-950/40'}`}>
                     {node.status === NodeStatus.ERROR ? (
                       ...
                       type === DataType.TEXT ? (
                        typeof res === 'object' ? <div className="space-y-1">{Object.entries(res || {}).map(([k,v]) => <p key={k} className="text-[9px] text-slate-400 truncate"><span className="text-indigo-400">{k}:</span> {v as string}</p>)}</div> : <p className="text-[10px] text-slate-400 line-clamp-4">{res}</p>
                     ) : type === DataType.IMAGE ? <img src={Array.isArray(res) ? res[0] : res} className="w-full h-full object-cover rounded-lg" /> :
