      ...
      <div className="bg-slate-900 border border-slate-800 rounded-[40px] shadow-2xl w-full max-w-5xl h-full flex flex-col relative overflow-hidden">
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <div className="flex flex-col gap-1">
              ...
              {expandedResultData.type === DataType.TEXT && (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${
                    isEditingResult ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-800 text-slate-400 hover:text-white'
                  }`}
                >
            ...
            {isEditingResult && (
              <p className="text-[10px] text-indigo-400 font-bold uppercase animate-pulse">{t('manual_edit_hint')}</p>
            )}
          </div>
        ...
        <div className="flex-1 p-12 overflow-y-auto flex items-center justify-center custom-scrollbar">
          {expandedResultData.type === DataType.TEXT ? (
            isEditingResult ? (
              ...
              <p className="text-lg leading-relaxed max-w-3xl whitespace-pre-wrap selection:bg-indigo-500/20">
